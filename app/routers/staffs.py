...
@router.put("/{staff_id}/photo_path", response_model=schemas.Staff)
async def update_staff_photo_path(staff_id: int, file: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    db_staff = crud.update_staff_photo_path(db=db, staff_id=staff_id, photo_url=public_url, filename=file.filename)
    if db_staff is None:
